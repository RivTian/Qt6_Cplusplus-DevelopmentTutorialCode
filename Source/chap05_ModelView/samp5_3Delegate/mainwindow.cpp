...
void MainWindow::iniModelData(QStringList &aFileContent)
{
    ...
    //    QStringList headerList=header.split(QRegExp("\\s+"),QString::SkipEmptyParts);//Qt5中的代码
    QStringList headerList=header.split(QRegularExpression("\\s+"),Qt::SkipEmptyParts);     //Qt6中的代码
    ...
    for (int i=1;i<rowCnt;i++)
    {
        ...
        //一个或多个空格、TAB等分隔符隔开的字符串， 分解为一个StringList
        //        QStringList tmpList=aLineText.split(QRegExp("\\s+"),QString::SkipEmptyParts);//Qt5中的代码
        QStringList tmpList=aLineText.split(QRegularExpression("\\s+"),Qt::SkipEmptyParts);////Qt6中的代码
...
{//模型数据导出到PlainTextEdit显示
    ...
    for (i=0;i<m_model->rowCount();i++)
    {
        ...
        aItem=m_model->item(i,j); //最后一行是逻辑型
